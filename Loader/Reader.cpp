		...
		if (reader.is_open())
		{
			...
			for (int mI = 0; mI < readStart.nrOfMeshes; mI++)
			{
				...
				for (int i = 0; i < readMesh.nrOfBlendShapes; i++)
				{
					...
					m_scene.m_blendshapes.emplace_back(acjlBS);
				}
			...
			for (int cI = 0; cI < readStart.nrOfCams; cI++)
			{
				...
				m_scene.m_cams.emplace_back(readCam);
				if (printData)
				{
